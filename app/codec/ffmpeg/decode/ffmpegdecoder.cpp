...
bool FFmpegDecoder::OpenInternal()
{
  if (instance_.Open(stream().filename().toUtf8(), stream().stream())) {
    ...
    if (s->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      ...
      native_pix_fmt_ = GetVideoParamsFormatFromAVFormat(ideal_pix_fmt_);
      native_channel_count_ = GetVideoParamsChannelCountFromAVFormat(ideal_pix_fmt_);
    ...
    for (unsigned int i=0;i<fmt_ctx->nb_streams;i++) {
      ...
      if (decoder
        ...
        if (avstream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
          ...
          stream.set_format(GetVideoParamsFormatFromAVFormat(compatible_pix_fmt));
          stream.set_channel_count(GetVideoParamsChannelCountFromAVFormat(compatible_pix_fmt));
